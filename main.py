...
import chromadb
import langchain
from authentication import openai_auth
...
def gen_questions(**kwargs):
  ...
  q_dic = kwargs
  a_ls = a_dic[q_dic["a_no"]] # transform the answer number into the answer list with True/False.
...
if __name__ == "main":
    openai_auth()
    CSV_PATH = "~/cpa-audit/data/"

    df_cpa = pd.read_excel(f"{CSV_PATH}/CPA_AUDIT.csv")
